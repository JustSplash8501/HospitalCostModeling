...
repository: https://github.com/JustSplash8501/HospitalCostModeling
execute:
  warning: false
  message: false
  echo: true
...
hospitaldata = pd.read_csv(DATA_PATH)
hospitaldata.head()
```
...
```{python}
from src.models.mlm import compute_vif
...
from src.models.mlm import adj_r2_score
...
The linear model explains approximately 78% of the variance in insurance costs with a mean absolute error (MAE) of $4181.29, but shows violations of several assumptions necessary for an interpretable and accurate model ultimately limiting its predictive accuracy. To address this, a model, Random Forest tree-based model, which can capture non-linear relationships will be tested to determine if it can be more accurate in prediction of the target variable.
...
study = optuna.create_study(
    direction="maximize", sampler=TPESampler(n_startup_trials=5)
)
...
# Calculate and plot best values
best_values = []
...
While the R² of the tuned random forest model did not improve compared to the base model, the MAE of the model was improved from 3192.66 to 2533.98 on the test dataset suggesting a noticeable improvement. Given this improvement, the tuned model should be statistically tested against the multi-linear regression model to determine which method statistically produces less error in prediction.
...
# Setup models folder and path
models_dir = Path("models")
models_dir.mkdir(parents=True, exist_ok=True) 
model_path = models_dir / "tuned_rf_model.pkl"
